...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function loadDashboard() {
      const [products, orders, stock, supplies] = await Promise.all([
      ...
      const delivered = Array.isArray(orders)
        ? orders.filter((o: { status: string }) => o.status === "entregue")
        : [];
      const pending = Array.isArray(orders)
        ? orders.filter((o: { status: string }) => o.status === "pendente")
        : [];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-800 mb-4">Produtos com Estoque Baixo</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-800 mb-4">Insumos com Estoque Baixo</h2>
